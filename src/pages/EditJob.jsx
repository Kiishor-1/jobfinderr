...
import { getJobById, updateJob } from "../slices/jobSlice";  // Import thunks
...
export default function EditJob() {
    ...
    const { currentJob, isLoading, error } = useSelector((state) => state.jobs); // Access Redux states
    ...
    useEffect(() => {
        if (id) {
            dispatch(getJobById(id)); // Dispatch Redux thunk to fetch job details
        }
    }, [id, dispatch]);
    ...
    // Populate the form once the job data is loaded
    useEffect(() => {
        if (currentJob) {
    ...
    // Handle form submission and update the job
    const onSubmit = async (data) => {
        const updatedData = {
            ...
            skills: skills, // Add updated skills
        };

        // Dispatch updateJob thunk and handle success or error using toast
